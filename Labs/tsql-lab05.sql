...
FROM Sales.Orders
WHERE orderdate <> DATEFROMPARTS(YEAR(orderdate), 12, 31) AS endofyear
...
drop view if exists dbo.totalquantity;
go
create view totalquantity as
  select o.empid, year(o.orderdate) as orderyear, sum(od.qty) as sum_qty
  from sales.orders o inner join sales.orderdetails od
  on o.orderid = od.orderid
  group by o.empid, year(o.orderdate);
go

select * from totalquantity order by empid, orderyear;
go
...
-- Write a query against Sales.VEmpOrders (renamed as dbo.totalquantity)
-- that returns the running qty for each employee and year
-- Tables involved: TSQLV4 database, Sales.VEmpOrders view

select t.empid, t.orderyear, t.sum_qty,
  (select sum(t1.sum_qty) as runqty
   from totalquantity t1
   where t.empid = t1.empid and t1.orderyear <= t.orderyear) as runqty
from totalquantity t
order by t.empid, t.orderyear;
